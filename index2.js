...
res.setHeader('Access-Control-Allow-Methods','GET,POST,PUT,PATCH,DELETE');
res.setHeader('Access-Control-Allow-Methods','Content-Type','Authorization');
con.query(`SELECT * FROM magti.dmc11 where ALD_PURT_NUMBER like'${zieba}'`,function(errr,result2,field){ 
res.send(result2)
// console.log(req.url)
})
...
const put_data=(req,res)=>{
    ...
    console.log(req.params.worker3)
    console.log(req.params.seriuli)
 ...
 con.query(` INSERT INTO magti.work (TP,Worker1,Worker2,Worker3,PART_NUMBER,ALD_PURT_NUMBER,SERIAL_NUMBER,location) 
             VALUES (${req.params.TP1}, '${req.params.worker1}',
                    '${req.params.worker2}','${req.params.worker3}',
                    '${req.params.part_new}','${req.params.ald_part}',
                    '${req.params.seriuli}','${req.params.location}')`,function(errr,result2,field){ 
        // res.send(result2)
        }) 
...
app.route('/new_data/:TP1?/:worker1?/:worker2?/:worker3?/:seriuli?/:part_new?/:ald_part?/:location?')   
   .get(put_data)         
